...
test "testing_with_simple_nodes" {
    ...
    var n1: simpleNode, var n2: simpleNode, var n3: simpleNode = .{simpleNode.init(10), simpleNode.init(20), simpleNode.init(30) };
    
    var queue = FIFOType(simpleNode){.name = "my_test_queue"};

    try std.testing.expect(queue.empty());
    try std.testing.expectEqualStrings(queue.name.?, "my_test_queue");
