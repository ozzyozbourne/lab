...
const SizePrecision = enum { exact, inexact };

inline fn copy_disjoint(comptime precision: SizePrecision, comptime T: type, target: []T, source: []const T) void {
    switch (precision) {
...
inline fn disjoint_slices(comptime A: type, comptime B: type, a: []const A, b: []const B) bool {
...
pub fn RingBufferType(comptime T: type, comptime buffer_type: union(enum) { array: usize, slice }) type {
    return struct {
