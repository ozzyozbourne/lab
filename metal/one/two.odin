...
build_buffers :: proc(
	...
	vertex_buffer, index_buffer, instance_buffer: ^MTL.Buffer,
) {
	s :: 0.5
	...
	instance_buffer =
	device->newBuffer(NUM_INSTANCES * size_of(Instance_Data), {.StorageModeManaged})
	return
}
...
metal_main :: proc() -> (err: ^NS.Error) {
	...
	SDL.setenv("METAL_DEVICE_WRAPPER_TYPE", "3", 0)
	SDL.Init({.VIDEO})
	defer SDL.Quit()

	window := SDL.CreateWindow(
		"Metal in Odin - 08 Compute",
	...
	for quit := false; !quit; {
...

		{
			...
			camera_data.perspective_transform = glm.mat4Perspective(
				glm.radians_f32(45),
				aspect_ratio,
				0.03,
				500,
